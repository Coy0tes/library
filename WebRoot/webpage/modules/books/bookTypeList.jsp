	...
	var validateForm;
		...
		function doSubmit(){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
			...
			if(length == 0){
				...
				top.layer.msg("只能选择一项！");
				code = "02";
			}else{
				code = "00";
				id = $("#checked").val();
				name = $("#name").text()+" "+$("#remarks").text();
			}
...
<body class="gray-bg">
	...
	<ol class="breadcrumb">
			    <li>
			 	   <a href="${ctx}/booktype/bookType/bookTypeList?parent=${bookType.id }" title="分类号查询"><font style="color:blue">分类号首页</font></a>
			    </li>
		...
		<div class="section category-items text-center">
		    ...
		    <c:forEach items="${list }" var="bookType">
		    	<div class="col-sm-3 panel panel-default categorydiv">
			        <a  href="${ctx}/booktype/bookType/bookTypeList?parent=${bookType.id }" title="${bookType.remarks }">
					    <div class="category-item panel-body" class="mousesj">
				    		<span hidden="hidden">${bookType.id }</span>
				    		<c:if test="${num == 1}">
						    	<div><input type="checkbox" name="checkbox" id="checked" value="${bookType.id}" class="i-checks"></div>
				    		</c:if>
				            <div><span id="name" class="category-code">${bookType.name }</span></div>
				            <span id="remarks" class="category-title text-overflow">${fns:abbr(bookType.remarks,30)}</span>
					    </div>
			        </a>
				</div>
