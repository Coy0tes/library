...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			// 学生信息事件，输入学号后，查询是否有该学生
			$("#stunum").change(function(){
				var student = $("#stunum").val();	
				$.ajax({
					url:"${ctx}/bookstudent/bookStudent/getByStudent",
					...
					success:function(data){
						...
						// 11 该生不存在
						if(data.code == "99"){
							top.layer.msg("在借图书没有找到该生，请核实学号后再试！",{icon:2});
							ispush = false;
						}else if(data.code == "11"){
							top.layer.msg("没有找到该生信息，请核实学号！");
							ispush = false;
			...
			$("#ib").change(function(){
				...
				if(stunum){
					$.ajax({
						url:"${ctx}/bookstudentback/bookStudentBack/getByIsbn",
						data:{isbn:isbn,studentnum:stunum},
						type:"post",
						dataType:"json",
						success:function(data){
								if(data.bookStudentDetail == "00"){
									top.layer.msg("没有相关信息，请核实学生学号或者ISBN后重试", {icon:2});
									clean();
									ispush = false;
								}else{
									...
									$("#studentnum").val(data.bookStudentDetail.studentnum);
									$("#studentname").val(data.bookStudentDetail.studentname);
									$("#classname").val(data.bookStudentDetail.classname);
...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="bookStudentBack" action="${ctx}/bookstudentback/bookStudentBack/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		   		<!-- 图书信息 -->
		   		<tr>
		   			<td class="width-15 active"><label class="pull-right"><font style="color:red">*</font>学号：</label></td>
					<td class="width-35">
						<input id="stunum" name="stunum" htmlEscape="false"  placeholder="请先输入学号"  class="form-control required" />
					</td>
				...
				<tr>
					...
					<td class="width-35">
						<input id="classname" htmlEscape="false"    class="form-control " readonly="true"/>
					</td>
		  		</tr>
		 	</tbody>
		</table>
		...
		<br>
		<br>
		<div style="text-align:center;margin-top:20px;"><font style="color:red">*</font>温馨提示：<strong>请先输入学生学号，然后点击ISBN号输入框，扫码后即可查询该学生借阅信息</strong></div>
	</form:form>
</body>
</html>