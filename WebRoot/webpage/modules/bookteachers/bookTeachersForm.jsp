...
<head>
	...
	<script type="text/javascript">
		...
		function btn(){
			var bh = $(".bh").val();
			$.ajax({
				url:"${ctx}/bookteachers/bookteachers/getByBh",
				data:{"teachers.bh":bh},
				type:"post",
				datatype:"json",
				success:function(data){
					...
					if(data.teachers){
						...
						$("input[name='teachers.id']").val(data.teachers.id);
						$("input[name='teachers.bh']").val(data.teachers.bh);
						$("input[name='teachers.name']").val(data.teachers.name);
						...
						if(data.bookStudent!=0){
							var dataBookteachers = data.bookStudent.bookStudentDetailList;
							
							// 在借书籍子表信息
							xiangqing(dataBookteachers);
							
						}
						
						// 使用隐藏域存放当前学生借书的数量，跟系统参数中最大借书数量
						// 将该教师的借书数量跟参数的最多借书数量附给隐藏域，查询书籍的时候调用该教师的借书数量来做判断，防止下一次借书时超过限制但是会借书成功
						// 隐藏域中学生借书数量是动态变化的，先从后台查询该教师借了几本书，然后前台添加时再加1
						$("#hiddenteachersBooksShuliang").val(data.teachersBooksShuliang);
						$("#hiddenCanshuMaxNum").val(data.canshu.booknum);
						var hiddenteachersBooksShuliang = $("#hiddenteachersBooksShuliang").val(); 
						// 根据学生借书数量，判断是否还能继续借书
						if(data.canshu.booknum > hiddenteachersBooksShuliang){
							...
							top.layer.msg("该教师此次借书已达到规定的"+data.canshu.booknum+"本数量，请该教师还书后再借！", {icon:2});
						}
						...
						//如果没有找到学生信息，重置为空
						top.layer.msg("没有找到该教师信息,请核实工号后重试！", {icon:2});
						$("#id").val("");
						$("input[name='teachers.id']").val("");
						$("input[name='teachers.bh']").val("");
						$("input[name='teachers.name']").val("");
						$("#yajin").val("");
					}
				}
			}); 
		...
		// 借书不成功，当前数量处理，-1
		function bookByNum(){
			var hiddenteachersBooksShuliang = parseInt($("#hiddenteachersBooksShuliang").val());
			hiddenteachersBooksShuliang -=1;
			$("#hiddenteachersBooksShuliang").val(hiddenteachersBooksShuliang);
		}
		...
		// 学生借书详情
		function xiangqing(dataBookteachers){
			//加载学生已借书籍信息
			for (var i=0; i<dataBookteachers.length; i++){
				addRow('#bookStudentHistoryList', bookStudentHistoryRowIdx, bookStudentHistoryTpl, dataBookteachers[i]);
		...
		function maxbook(idx){
			var isMax = true;
			// 隐藏域动态赋值
			var hiddenteachersBooksShuliang1 = $("#hiddenteachersBooksShuliang").val();
			var hiddenteachersBooksShuliang1 = parseInt(hiddenteachersBooksShuliang1);
				hiddenteachersBooksShuliang1+=1;
				
			var hiddenCanshuMaxNum = $("#hiddenCanshuMaxNum").val();
			
			if(hiddenteachersBooksShuliang1<=hiddenCanshuMaxNum){
				
				// 借书数量+1
				$("#hiddenteachersBooksShuliang").val(hiddenteachersBooksShuliang1);
				
			}else{
				top.layer.msg("超出规定的书数量！", {icon:2});
				// 调用清空行
				resetRow(idx);
				isMax = false;
			}
...
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="bookStudent" action="${ctx}/bookteachers/bookteachers/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input type="hidden" id="hiddenteachersBooksShuliang">
		<input type="hidden" id="hiddenCanshuMaxNum">
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>教师工号：</label></td>
					<td class="width-35">
						<form:hidden path="teachers.id" htmlEscape="false"    class="form-control required teachersid"/>
						<form:input path="teachers.bh" htmlEscape="false" style="width:50%" placeholder="输入教师工号后回车即可查询" onkeydown="keyDown(event)" class="form-control bh"/>
		  		...
		  		<tr>
					...
					<td class="width-35">
						<form:input path="teachers.name" htmlEscape="false"    class="form-control "/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><strong>请先输入教师工号，按回车键自动加载完教师信息，确认无误后可直接扫描图书的ISBN号，等图书信息自动加载后，可继续扫描，确认信息无误后可点击确定提交</strong></td>
				</tr>
		 	</tbody>
		</table>
		...
		<div class="tabs-container">
            <ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#tab-1" aria-expanded="true">教师借书：</a>
                </li>
				<li class=""><a data-toggle="tab" href="#tab-2" aria-expanded="false">教师在借书籍：</a>
                </li>
            </ul>
