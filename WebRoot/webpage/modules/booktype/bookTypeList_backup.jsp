...
<body class="gray-bg">
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tbody>
		<c:forEach items="${page.list}" var="bookType">
			<tr>
				<td> <input type="checkbox" id="${bookType.id}" class="i-checks"></td>
				<td><a  href="#" onclick="openDialogView('查看图书分类', '${ctx}/booktype/bookType/form?id=${bookType.id}','800px', '500px')">
					${bookType.remarks}
				</a></td>
				<td>
					${bookType.parent.id}
				...
				<td>
					<shiro:hasPermission name="booktype:bookType:view">
						<a href="#" onclick="openDialogView('查看图书分类', '${ctx}/booktype/bookType/form?id=${bookType.id}','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="booktype:bookType:edit">
    					<a href="#" onclick="openDialog('修改图书分类', '${ctx}/booktype/bookType/form?id=${bookType.id}','800px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="booktype:bookType:del">
						<a href="${ctx}/booktype/bookType/delete?id=${bookType.id}" onclick="return confirmx('确认要删除该图书分类吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
