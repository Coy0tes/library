	...
	<div class="col-sm-12">
		<div class="pull-left">
			<shiro:hasPermission name="classes:classes:add">
				<table:addRow url="${ctx}/classes/classes/form" title="班级管理" height="40%" width="40%"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="classes:classes:edit">
			    <table:editRow url="${ctx}/classes/classes/form" title="班级管理" id="contentTable" height="40%" width="40%"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<c:forEach items="${page.list}" var="classes">
			<tr>
				...
				<td><a  href="#" onclick="openDialogView('查看班级管理', '${ctx}/classes/classes/form?id=${classes.id}','40%', '40%')">
					${classes.name}
				</a></td>
				<td>
					<shiro:hasPermission name="classes:classes:view">
						<a href="#" onclick="openDialogView('查看班级管理', '${ctx}/classes/classes/form?id=${classes.id}','40%', '40%')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="classes:classes:edit">
    					<a href="#" onclick="openDialog('修改班级管理', '${ctx}/classes/classes/form?id=${classes.id}','40%', '40%')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="classes:classes:del">
						<a href="${ctx}/classes/classes/delete?id=${classes.id}" onclick="return confirmx('确认要删除该班级管理吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
