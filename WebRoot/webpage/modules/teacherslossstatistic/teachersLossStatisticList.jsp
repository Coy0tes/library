...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="bookStudentHistory" action="${ctx}/teacherslossstatistic/teacherslossstatistic/" method="post" class="form-inline">
	...
	<div class="col-sm-12">
		<div class="pull-left">
			<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:add">
				<table:addRow url="${ctx}/teacherslossstatistic/teacherslossstatistic/form" title="教师借阅历史记录" width="85%" height="85%"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:edit">
			    <table:editRow url="${ctx}/teacherslossstatistic/teacherslossstatistic/form" title="教师借阅历史记录" id="contentTable" width="85%" height="85%"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:del">
				<table:delRow url="${ctx}/teacherslossstatistic/teacherslossstatistic/deleteAll" id="contentTable"></table:delRow><!-- 删除按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:import">
				<table:importExcel url="${ctx}/teacherslossstatistic/teacherslossstatistic/import"></table:importExcel><!-- 导入按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:export">
	       		<table:exportExcel url="${ctx}/teacherslossstatistic/teacherslossstatistic/export"></table:exportExcel><!-- 导出按钮 -->
	       	</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<c:forEach items="${page.list}" var="bookStudentHistory">
			<tr>
				...
				<td>
					<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:view">
						<a href="#" onclick="openDialogView('查看教师借阅历史记录', '${ctx}/teacherslossstatistic/teacherslossstatistic/form?id=${bookStudentHistory.id}','85%', '85%')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:edit">
    					<a href="#" onclick="openDialog('修改教师借阅历史记录', '${ctx}/teacherslossstatistic/teacherslossstatistic/form?id=${bookStudentHistory.id}','85%', '85%')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="teacherslossstatistic:teacherslossstatistic:del">
						<a href="${ctx}/teacherslossstatistic/teacherslossstatistic/delete?id=${bookStudentHistory.id}" onclick="return confirmx('确认要删除该教师借阅历史记录吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
